...
def _git_subject(cwd: Path) -> tuple[str | None, bool | None]:
...
class VerificationService:
    ...
    def execute(self, plan: VerificationPlan, *, task_id: str, cwd: str | Path = ".",
        ...
        with self.db.tx(immediate=True) as con:
            ...
            (run_id,plan_hash,task_id,subject_sha or '',1 if dirty else 0,str(cwdp),env_hash,utc_now()))
            # Store whether dirty was allowed in the plan metadata
            dirty_allowed = 1 if (not require_clean and dirty) else 0
            con.execute(
                "UPDATE verification_runs_v2 SET artifact_set_json=? WHERE verification_run_id=?",
                (json.dumps({"dirty_allowed": dirty_allowed}), run_id),
            )
    ...
    def _collect_artifacts(self, plan: VerificationPlan, cwd: Path, run_id: str) -> tuple[list[dict[str, Any]],list[str]]:
        ...
        for pattern in plan.artifacts:
            matches=[]
            # If pattern is already absolute, use it directly; otherwise join with cwd
            if Path(pattern).is_absolute():
                candidates = [pattern]
            else:
                candidates = sorted(glob.glob(str(cwd/pattern)))
            for raw in candidates:
                p=Path(raw)
                if p.is_file():
                    row={"path":str(p.resolve()),"sha256":sha256(p.read_bytes()).hexdigest(),"bytes":p.stat().st_size}
    ...
    def verify_evidence(self, run_id: str, *, allow_dirty: bool = False) -> tuple[bool, str]:
        ...
        if run.get("subject_dirty") and not allow_dirty:
            return False,"dirty_subject"
