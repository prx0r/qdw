...
class BuildCertificateV2:
    ...
    def issue(self, verification_run_id: str, *, ledger_root: str = "", allow_dirty: bool = False) -> dict:
        ok,reason=self.verification.verify_evidence(verification_run_id, allow_dirty=allow_dirty)
        if not ok:
        ...
        if current_sha and run.get("subject_git_sha") and current_sha!=run["subject_git_sha"]:
            raise ValueError("subject changed after verification")
        if current_dirty and not allow_dirty:
    ...
    def verify(self, build_certificate_id: str) -> tuple[bool,str]:
        ...
        ok,reason=self.verification.verify_evidence(row["verification_run_id"], allow_dirty=True)
