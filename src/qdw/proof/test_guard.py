"""AST-based test guard — detects fake-green patterns."""
    ...
    def visit_FunctionDef(self, node: ast.FunctionDef):
        if node.name.startswith("test_"):
            meaningful = [
                ...
                if not isinstance(x, (ast.Pass, ast.Expr))
                or not (isinstance(x, ast.Expr) and isinstance(x.value, ast.Constant)
...
def scan_test_file(path: str | Path) -> list[Finding]:
    ...
    try:
        ...
        return [Finding(str(p), e.lineno or 0, "SYNTAX", "test file does not parse")]
    v = _Visitor(str(p))
    v.visit(tree)
    return v.findings
...
def scan_test_tree(path: str | Path) -> list[Finding]:
    out: list[Finding] = []
    for p in Path(path).rglob("test_*.py"):
        out.extend(scan_test_file(p))
    return out
