"""Hermes executor — bounded worker with structured return contract."""
...
class HermesExecutor:
    ...
    def execute(self, request: ExecutionRequest) -> ExecutionResult:
        ...
        prompt = (
            "Execute this bounded Factory OS work node. "
            "Return ONE JSON object matching return_contract and no markdown fences.\n"
        ...
        try:
            final = json.loads(p.stdout.strip())
        except json.JSONDecodeError:
            return ExecutionResult(
                False, "invalid_result", stdout=p.stdout, stderr=p.stderr, exit_code=p.returncode,
            )
        status = final.get("status", "failed")
        return ExecutionResult(
            status == "ok", status, final=final,
