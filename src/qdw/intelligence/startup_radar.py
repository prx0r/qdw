"""StartupRadar — company event tracking."""
...
class StartupRadar:
    ...
    def record_company_event(self, company_name: str, event_type: str, event_at: str, *,
                             external_key: str | None = None, amount_usd: float | None = None,
                             stage: str | None = None, attributes: dict | None = None,
                             observation_id: str | None = None) -> str:
        company_id = self.world.upsert_entity(
            "company", company_name, external_key=external_key, attributes=attributes or {},
        )
        eid = new_id("startup")
        with self.db.tx(immediate=True) as con:
            con.execute(
                """INSERT INTO startup_events(startup_event_id,company_entity_id,event_type,event_at,
                amount_usd,stage,attributes_json,observation_id,created_at)
                VALUES(?,?,?,?,?,?,?,?,?)""",
                (eid, company_id, event_type, event_at, amount_usd, stage,
                 canonical_json(attributes or {}).decode(), observation_id, utc_now()),
            )
    ...
    def recent(self, event_type: str | None = None, limit: int = 50) -> list[dict]:
        ...
        args: list = []
        if event_type:
            q += " WHERE s.event_type=?"
            args.append(event_type)
        q += " ORDER BY s.event_at DESC LIMIT ?"
        args.append(limit)
