"""AlternativeAPI — find substitutes; if none qualify, produce a build candidate."""
...
from qdw.intelligence.opportunities import OpportunitySynthesizer
from qdw.intelligence.stack_oracle import ResourceRecommendation, StackOracle
...
class AlternativeAPI:
    ...
    def match(self, capability_key: str, *, current_resource_id: str | None = None,
        ...
        recs = self.stack.recommend(
            capability_key, required_attributes=required_attributes,
        ...
        oid = None
        if create_gap:
            oid = self.synth.api_gap(
                capability_key, problem_key=f"alternative:{capability_key}",
