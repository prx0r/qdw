"""HumanQueue — durable human-in-the-loop with idempotency."""
...
class HumanQueue:
    ...
    def request(self, action_type: str, title: str, instructions: dict[str, Any], *,
                idempotency_key: str, product_id: str | None = None,
                factory_run_id: str | None = None, work_node_id: str | None = None,
                estimated_cost_usd: float | None = None) -> str:
        with self.db.tx(immediate=True) as con:
            old = con.execute(
                "SELECT action_id FROM human_actions WHERE idempotency_key=?", (idempotency_key,)
            ).fetchone()
            if old:
                return old["action_id"]
            aid = new_id("human")
            con.execute(
                """INSERT INTO human_actions(action_id,product_id,factory_run_id,work_node_id,action_type,status,
        ...
        self.ledger.append("human.requested", "human_action", aid,
                           {"action_type": action_type, "product_id": product_id,
    ...
    def _transition(self, action_id: str, new_status: str, payload: dict[str, Any] | None = None,
        ...
        with self.db.tx(immediate=True) as con:
            ...
            if new_status in {"APPROVED", "DECLINED", "CANCELLED"}:
                con.execute(
                    """UPDATE human_actions SET status=?,decided_at=?,decision_json=? WHERE action_id=?""",
    ...
    def pending(self) -> list[dict]:
        with self.db.connect() as con:
            rows = con.execute(
                """SELECT * FROM human_actions WHERE status IN ('REQUESTED','APPROVED')
                ORDER BY requested_at""",
            ).fetchall()
        out = []
        for r in rows:
            d = dict(r)
            d["instructions"] = json.loads(d.pop("instructions_json"))
