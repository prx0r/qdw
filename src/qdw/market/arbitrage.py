"""Arbitrage estimation — UNKNOWN stays UNKNOWN, never zero."""
...
def estimate(i: ArbitrageInputs) -> ArbitrageResult:
    legs = [
        i.source_price, i.target_price, i.marketplace_fees, i.payment_fees,
        i.shipping, i.expected_returns, i.taxes_duties, i.fx_buffer,
    ]
    ...
    contribution = (
        i.target_price.amount - i.source_price.amount - i.marketplace_fees.amount
        - i.payment_fees.amount - i.shipping.amount - i.expected_returns.amount
        - i.taxes_duties.amount - i.fx_buffer.amount
    )
