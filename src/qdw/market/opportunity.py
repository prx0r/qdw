"""Opportunity scoring — agentic opportunity score with frozen snapshots."""
...
def agentic_opportunity_score(f: OpportunityFeatures) -> float:
    ...
    if any(x <= 0 for x in positives):
        return 0.0
    p = 1.0
    for x in positives:
        p *= max(1e-9, min(1, x))
    base = p ** (1 / len(positives))
    friction = 0.45 * f.competition + 0.30 * f.integration_cost + 0.25 * f.failure_risk
