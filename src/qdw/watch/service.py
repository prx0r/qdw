"""WatchService — stores re-evaluation conditions, does not silently mutate ideas."""
...
class WatchService:
    ...
    def add(self, subject_type: str, subject_id: str, trigger_type: str, condition: dict[str, Any]) -> str:
        ...
        with self.db.tx(immediate=True) as con:
            con.execute(
                """INSERT INTO watch_triggers(trigger_id,subject_type,subject_id,trigger_type,condition_json,
                status,created_at) VALUES(?,?,?,?,?,'ACTIVE',?)""",
                (tid, subject_type, subject_id, trigger_type, canonical_json(condition).decode(), utc_now()),
            )
        self.ledger.append("watch.created", "watch_trigger", tid,
    ...
    def record_evaluation(self, trigger_id: str, result: dict[str, Any]) -> None:
        with self.db.tx(immediate=True) as con:
            con.execute(
                "UPDATE watch_triggers SET last_evaluated_at=?,last_result_json=? WHERE trigger_id=?",
