"""IdeaLibrary — unused ideas, same-problem lookup, cross-form inspiration."""
...
class IdeaLibrary:
    ...
    def unused(self, limit: int = 100) -> list[dict]:
        with self.db.connect() as con:
            rows = con.execute(
                """SELECT * FROM ideas WHERE status IN ('PROPOSED','PASS','DORMANT')
    ...
    def same_problem(self, problem_key: str, *, exclude_idea_id: str | None = None) -> list[dict]:
        q = "SELECT * FROM ideas WHERE problem_key=?"
        args: list = [problem_key]
        if exclude_idea_id:
            q += " AND idea_id!=?"
            args.append(exclude_idea_id)
