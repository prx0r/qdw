...
class OpportunityStore:
    ...
    def create(self,*,kind:str,problem_key:str,title:str,thesis:str,features:dict[str,Any],
               evidence_refs:list[dict[str,str]],score:dict[str,Any],factory_hint:str|None=None)->str:
        feature_hash=hash_object(features)
        evidence_hash=hash_object(sorted(evidence_refs,key=lambda x:canonical_json(x)))
        oid=new_id("opp")
        now=utc_now()
        with self.db.tx(immediate=True) as con:
            con.execute("""INSERT INTO opportunities_global(opportunity_id,kind,problem_key,title,thesis,factory_hint,
            ...
            for ref in evidence_refs:
                con.execute("""INSERT OR IGNORE INTO opportunity_evidence(opportunity_id,observation_id,claim_id,
    ...
    def get(self,opportunity_id:str)->dict:
        with self.db.connect() as con:
            r=con.execute("SELECT * FROM opportunities_global WHERE opportunity_id=?",(opportunity_id,)).fetchone()
            if not r:raise KeyError(opportunity_id)
            d=dict(r)
...
class OpportunitySynthesizer:
    """Deterministic join layer. LLMs may later propose narratives, but evidence/score stays structured."""
    ...
    def from_pain_cluster(self,cluster_id:str,*,new_capability_resource_id:str|None=None,
        ...
        with self.db.connect() as con:
            ...
            if not c:raise KeyError(cluster_id)
        features={
        ...
        value=(features["need"]*.20+features["source_breadth"]*.10+features["recurrence"]*.15+
               features["intensity"]*.10+features["agent_solvability"]*.20+
               features["verifiability"]*.15+features["confidence"]*.10)
        evidence=[{"pain_cluster_id":cluster_id,"role":"problem"}]
        if new_capability_resource_id:evidence.append({"resource_id":new_capability_resource_id,"role":"enabler"})
        return self.store.create(kind="agentic_problem",problem_key=c["problem_key"],
            title=f"Solve: {c['title']}",thesis=c["summary"],features=features,evidence_refs=evidence,
            score={"expected_value_proxy":value,"confidence":c["confidence"]},factory_hint=factory_hint)

    def api_gap(self,capability_key:str,*,problem_key:str,current_resource_id:str|None,
                evidence_resources:list[str])->str:
        features={"capability_gap":1.0,"known_candidates":len(evidence_resources),
        ...
        return self.store.create(kind="api_gap",problem_key=problem_key,
            title=f"API gap: {capability_key}",
            thesis="No candidate satisfies the declared capability constraints; evaluate whether QDW can build one.",
            features=features,evidence_refs=refs,score={"expected_value_proxy":.55,"confidence":.5},
            factory_hint="api")
