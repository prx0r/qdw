...
class DistributionRegistry:
    ...
    def register_manifest(self,path:str|Path)->str:
        ...
        with self.db.tx(immediate=True) as con:
            con.execute("""INSERT INTO distribution_surfaces(surface_id,name,kind,manifest_json,status,created_at)
                VALUES(?,?,?,?,?,?)
    ...
    def eligible(self,product_type:str)->list[dict[str,Any]]:
        ...
        out=[]
        for r in rows:
            d=dict(r);m=json.loads(d["manifest_json"])
            if product_type in m.get("product_types",[]) or "*" in m.get("product_types",[]):
                d["manifest"]=m;out.append(d)
        return out

    def record_publication(self,product_id:str,surface_id:str,status:str,*,external_ref:str|None=None,
                           evidence:dict[str,Any]|None=None)->str:
        pid=new_id("pub")
        with self.db.tx(immediate=True) as con:
            con.execute("""INSERT INTO publications(publication_id,product_id,surface_id,status,external_ref,evidence_json,
