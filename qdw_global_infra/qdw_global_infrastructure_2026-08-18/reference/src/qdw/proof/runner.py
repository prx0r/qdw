...
def _git(cwd:Path)->tuple[str|None,bool|None]:
    try:
        ...
        if sha.returncode!=0:return None,None
...
class VerificationRunner:
    """Only this runner computes PASS/FAIL. Agents write specs; processes produce evidence."""
    ...
    def run(self,task_id:str,argv:Sequence[str],*,cwd:str|Path=".",timeout:int=300,
        ...
        rdir=self.runs_dir/rid;rdir.mkdir()
        started=_now();t0=time.monotonic()
        try:
            p=subprocess.run(list(argv),cwd=cwdp,capture_output=True,timeout=timeout,
        ...
        duration=int((time.monotonic()-t0)*1000);finished=_now()
        outp=rdir/"stdout.log";errp=rdir/"stderr.log"
        outp.write_bytes(stdout);errp.write_bytes(stderr)
        gsha,dirty=_git(cwdp)
        receipt=CommandReceipt(rid,task_id,started,finished,duration,str(cwdp),tuple(argv),code,
