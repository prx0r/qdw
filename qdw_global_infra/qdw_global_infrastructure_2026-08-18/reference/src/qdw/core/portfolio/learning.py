...
class FactoryLearning:
    ...
    def update(self,factory_id:str,version:str,*,certified:bool,outcome_success:bool,
               utility:float,cost_usd:float)->None:
        with self.db.tx(immediate=True) as con:
            r=con.execute("""SELECT * FROM factory_stats WHERE factory_id=? AND factory_version=?""",
                          (factory_id,version)).fetchone()
            if not r:
                con.execute("""INSERT INTO factory_stats(factory_id,factory_version,runs,certified_runs,
                    success_alpha,success_beta,total_cost_usd,total_utility,updated_at)
                    VALUES(?,?,?,?,?,?,?,?,?)""",
                    (factory_id,version,1,1 if certified else 0,2 if outcome_success else 1,
                     1 if outcome_success else 2,cost_usd,utility,utc_now()))
            else:
                con.execute("""UPDATE factory_stats SET runs=runs+1,certified_runs=certified_runs+?,
    ...
    def posterior(self,factory_id:str,version:str)->FactoryPosterior|None:
        ...
        runs=max(1,r["runs"]);a=r["success_alpha"];b=r["success_beta"]
