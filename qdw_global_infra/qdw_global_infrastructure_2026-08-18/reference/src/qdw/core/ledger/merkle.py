...
def merkle_root(items:Sequence[bytes])->bytes:
    if not items: return _h(b"")
    if len(items)==1: return leaf_hash(items[0])
...
def inclusion_path(items:Sequence[bytes],index:int)->list[bytes]:
    if index<0 or index>=len(items): raise IndexError(index)
...
def verify_inclusion(item:bytes,index:int,tree_size:int,path:Sequence[bytes],expected_root:bytes)->bool:
    if tree_size<=0 or index<0 or index>=tree_size: return False
    h=leaf_hash(item); fn=index; sn=tree_size-1
    for p in path:
        if fn%2==1 or fn==sn: h=node_hash(p,h)
        else: h=node_hash(h,p)
        fn//=2; sn//=2
