...
from qdw.core.core import canonical_json, hash_object, new_id, sha256_hex, utc_now
...
class WorldStore:
    """Canonical entity/observation/claim/relation plane."""

    def __init__(self, db: Database, ledger: Ledger):
        self.db, self.ledger = db, ledger

    def register_source(self, source_id: str, family: str, name: str, *, terms_url: str | None=None,
                        config: dict[str, Any] | None=None) -> None:
        now=utc_now()
        with self.db.tx(immediate=True) as con:
            con.execute("""INSERT INTO source_connectors(source_id,family,name,config_json,terms_url,status,created_at,updated_at)
                VALUES(?,?,?,?,?,'ACTIVE',?,?)
    ...
    def upsert_entity(self, kind: str, canonical_name: str, *, external_key: str | None=None,
        ...
        with self.db.tx(immediate=True) as con:
            if external_key is not None:
                row=con.execute("SELECT entity_id FROM entities WHERE kind=? AND external_key=?",
                                (kind,external_key)).fetchone()
                if row: entity_id=row["entity_id"]
            if entity_id is None:
                entity_id=new_id("ent")
                con.execute("""INSERT INTO entities(entity_id,kind,canonical_name,external_key,attributes_json,status,created_at,updated_at)
                    VALUES(?,?,?,?,?,'ACTIVE',?,?)""",
                    (entity_id,kind,canonical_name,external_key,canonical_json(attributes or {}).decode(),now,now))
            else:
                con.execute("""UPDATE entities SET canonical_name=?,attributes_json=?,updated_at=? WHERE entity_id=?""",
                            (canonical_name,canonical_json(attributes or {}).decode(),now,entity_id))
            for alias in aliases or []:
                n=normalize_text(alias)
                if n:
                    con.execute("""INSERT OR IGNORE INTO entity_aliases(entity_id,alias,normalized_alias)
                        VALUES(?,?,?)""",(entity_id,alias,n))
    ...
    def record_source_result(self, result: SourceResult, *, observed_at: str | None=None) -> list[str]:
        """Persist success/failure distinctly. A failure never becomes an empty successful observation."""
        ...
        if not result.ok:
            ...
            with self.db.tx(immediate=True) as con:
                con.execute("""INSERT INTO observations(observation_id,source_id,source_item_id,source_family,
        ...
        if not result.items:
            ...
            oid=new_id("obs")
            with self.db.tx(immediate=True) as con:
                existing=con.execute("""SELECT observation_id FROM observations
                    WHERE source_id=? AND source_item_id=? AND content_hash=?""",
                    (result.source_id,source_item_id,content_hash)).fetchone()
        ...
        for i,item in enumerate(result.items):
            ...
            with self.db.tx(immediate=True) as con:
                existing=con.execute("""SELECT observation_id FROM observations
                    ...
                    (result.source_id,source_item_id,content_hash)).fetchone()
                if existing:
                    ids.append(existing["observation_id"]); continue
                con.execute("""INSERT INTO observations(observation_id,source_id,source_item_id,source_family,
    ...
    def add_claim(self, predicate: str, obj: Any, *, observation_id: str | None=None,
        ...
        with self.db.tx(immediate=True) as con:
            ...
            cid=new_id("clm")
            con.execute("""INSERT INTO claims(claim_id,observation_id,subject_entity_id,predicate,
    ...
    def relate(self, subject_id: str, predicate: str, object_id: str, *,
        ...
        rid=new_id("rel")
        with self.db.tx(immediate=True) as con:
            old=con.execute("""SELECT relation_id FROM relations WHERE subject_entity_id=? AND predicate=?
                AND object_entity_id=? AND supporting_claim_id IS ?""",
                (subject_id,predicate,object_id,supporting_claim_id)).fetchone()
            if old:return old["relation_id"]
            con.execute("""INSERT INTO relations(relation_id,subject_entity_id,predicate,object_entity_id,
    ...
    def entity(self, entity_id: str) -> dict[str, Any]:
        with self.db.connect() as con:
            ...
            d=dict(r); d["attributes"]=json.loads(d.pop("attributes_json"))
            return d
