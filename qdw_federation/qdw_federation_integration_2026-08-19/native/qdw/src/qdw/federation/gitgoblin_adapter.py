    ...
    def normalize(self,raw:dict,request:dict|None=None)->ExternalSnapshot:
        ...
        if schema!=self.protocol_version:
            return ExternalSnapshot(
              ...
              hash_object(request or {}),hash_object(raw),ExternalStatus.INCOMPATIBLE_PROTOCOL,
        ...
        return ExternalSnapshot(
          "gitgoblin","frontier_observations",schema,hash_object(request or {}),
          hash_object(raw),status,str(raw.get("generated_at") or utc_now()),
          {"cursor":raw.get("cursor"),"batch_digest":raw.get("batch_digest"),
           "observations":observations,"opportunity_proposals":proposals},
          source_revision=raw.get("source_revision"),adapter_version=self.adapter_version)

    def to_source_result(self,snapshot:ExternalSnapshot)->SourceResult:
        if snapshot.status in {
            ExternalStatus.UNAVAILABLE,ExternalStatus.FAILED,ExternalStatus.INCOMPATIBLE_PROTOCOL,
            ExternalStatus.UNAUTHORIZED
        }:
        ...
        for o in snapshot.normalized.get("observations",[]):
            ...
            items.append({
              ...
              "federated_ref":ref,
              "entity_key":o.get("entity_key"),"metric":o.get("metric"),
              "value":o.get("value"),"unit":o.get("unit"),
              "dimensions":o.get("dimensions") or {},
              "evidence":{
                 "source_system":"gitgoblin","authority":"OBSERVATION",
