...
class FederatedRouteRegistry:
    ...
    def register(self,b:CandidateBinding,snapshot_id=None,advisory_id=None):
        self.system.route_registry.register(b.route)
        xref=self.federation_store.put_ref(b.external_ref)
        now=utc_now()
        bd=hash_object({"route_id":b.route.route_id,"external_ref":b.external_ref,
                        "route_kind":b.route_kind,"snapshot":snapshot_id,
                        "advisory":advisory_id,"profile":b.profile})
    ...
    def reconcile_source(self,system_id:str,current_route_ids:set[str]):
        """Deactivate foreign routes absent from the newest authoritative snapshot/list."""
        with self.system.db.tx(immediate=True) as con:
            rows=con.execute("""SELECT b.route_id FROM federation_route_bindings b
              JOIN federated_refs r ON r.federated_ref_id=b.federated_ref_id
              JOIN route_definitions d ON d.route_id=b.route_id
              WHERE r.system_id=? AND d.active=1""",(system_id,)).fetchall()
            retired=[]
            for row in rows:
                rid=row["route_id"]
                if rid in current_route_ids:continue
                con.execute("UPDATE route_definitions SET active=0,updated_at=? WHERE route_id=?",
                            (utc_now(),rid))
                self.system.ledger.append_in_tx(con,"federation.route_retired","route",rid,
                                                {"system_id":system_id,"reason":"ABSENT_FROM_CURRENT_SOURCE"})
                retired.append(rid)
        self.system.route_registry.reload()
        return retired

    def binding(self,route_id):
        with self.system.db.connect() as con:
            r=con.execute("""SELECT b.*,r.system_id,r.object_type,r.object_id,r.object_version,
              ...
              ON r.federated_ref_id=b.federated_ref_id WHERE b.route_id=?""",(route_id,)).fetchone()
