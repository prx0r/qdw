...
class PersistentBanditStore:
    ...
    def get(self,cell_id:str,route:Route)->Posterior:
        with self.db.connect() as con:
            row=con.execute("SELECT alpha,beta FROM route_posteriors WHERE cell_id=? AND route_id=?",
                            (cell_id,route.route_id)).fetchone()
        ...
        with self.db.tx(immediate=True) as con:
            con.execute("""INSERT OR IGNORE INTO route_posteriors(
              cell_id,route_id,alpha,beta,updated_at) VALUES(?,?,?,?,?)""",
              (cell_id,route.route_id,prior.alpha,prior.beta,utc_now()))
            row=con.execute("SELECT alpha,beta FROM route_posteriors WHERE cell_id=? AND route_id=?",
                            (cell_id,route.route_id)).fetchone()
    ...
    def update(self,cell_id:str,route_id:str,success:bool,weight:float=1.0)->Posterior:
        with self.db.tx(immediate=True) as con:
            con.execute("""INSERT OR IGNORE INTO route_posteriors(
              cell_id,route_id,alpha,beta,updated_at) VALUES(?,?,1.0,1.0,?)""",
              (cell_id,route_id,utc_now()))
            col="alpha" if success else "beta"
            con.execute(f"UPDATE route_posteriors SET {col}={col}+?,updated_at=? WHERE cell_id=? AND route_id=?",
                        (weight,utc_now(),cell_id,route_id))
            row=con.execute("SELECT alpha,beta FROM route_posteriors WHERE cell_id=? AND route_id=?",
                            (cell_id,route_id)).fetchone()
        return Posterior(row["alpha"],row["beta"])
    def mean_and_lower(self,cell_id,route):
        p=self.get(cell_id,route);successes,trials=beta_pseudo_counts(p.alpha,p.beta)
        return p.mean,wilson_lower(successes,trials)
    def thompson(self,cell_id,route,rng=None):
        r=rng or random;p=self.get(cell_id,route);return r.betavariate(p.alpha,p.beta)

    def save_route(self,route:Route)->None:
        with self.db.tx(immediate=True) as con:
            con.execute("""INSERT INTO route_definitions(
              route_id,model_id,provider_id,active,free,input_per_m,output_per_m,fixed_request_cost_usd,
              context_tokens,tools_supported,json_supported,reliability,latency_ms,
              cheapest_paid_replacement_cost,created_at,updated_at,endpoint_id,account_id,
              prior_success,prior_confidence,breaker_open,quota_pressure,evidence_ids_json
            ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
            ...
            (route.route_id,route.model_id,route.provider_id,int(route.active),int(route.free),
             route.input_per_m,route.output_per_m,route.fixed_request_cost_usd,route.context_tokens,
             ...
             route.prior_confidence,int(route.breaker_open),route.quota_pressure,
             json.dumps(list(route.evidence_ids or []),sort_keys=True)))

    def load_routes(self,active_only:bool=True)->list[Route]:
        with self.db.connect() as con:
            sql="SELECT * FROM route_definitions"+(" WHERE active=1" if active_only else "")+" ORDER BY route_id"
        ...
        for r in rows:
            out.append(Route(
              ...
              fixed_request_cost_usd=r["fixed_request_cost_usd"],context_tokens=r["context_tokens"],
...
class RouteRegistry:
    def __init__(self,store:PersistentBanditStore):
        self.store=store;self._routes={r.route_id:r for r in store.load_routes()}
    def register(self,route):
        self.store.save_route(route);self._routes[route.route_id]=route
    def active(self):return [self._routes[k] for k in sorted(self._routes) if self._routes[k].active]
    def get(self,route_id):return self._routes[route_id]
    def reload(self):self._routes={r.route_id:r for r in self.store.load_routes()}
