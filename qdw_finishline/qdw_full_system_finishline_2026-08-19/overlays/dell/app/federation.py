...
def candidate_dict(c,a=None):
    return {
      "offer_id":c.offer_id,"model_id":c.model_id,"provider_id":c.provider_id,
      "endpoint_id":c.endpoint_id,
      "input_per_m":c.input_per_m,"output_per_m":c.output_per_m,"free":c.free,
      "context_tokens":c.context_tokens,"max_output_tokens":c.max_output_tokens,
      ...
      "quota_rpd":c.quota_rpd,
      "estimated_cost":c._workload_cost,
      "score":a.score if a else None,
      "evidence_coverage":a.evidence_coverage if a else None,
      "confidence":a.confidence if a else None,
    }

def federation_resolve(raw:dict[str,Any],offers:list[dict],endpoints:list[dict]|None=None):
    ...
    for c in candidates:
        ...
        if reasons:excluded.append({"candidate":candidate_dict(c),"reasons":reasons})
        else:
    ...
    return {
      ...
      "authority":"ADVISORY",
      "candidates":[candidate_dict(c,a) for c,a in eligible],
      "recommended":candidate_dict(*eligible[0]) if eligible else None,
      "excluded":excluded,
      "decision":{
...
def load_endpoints(conn):
    exists=conn.execute("""SELECT 1 FROM sqlite_master WHERE type='table' AND name='serving_endpoints'""").fetchone()
    if not exists:return []
    return [dict(r) for r in conn.execute("SELECT * FROM serving_endpoints").fetchall()]
